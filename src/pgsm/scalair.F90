...
subroutine scalair(cnom, iheur, nniv, niveaux)
    use app
    use files, only : nRecords
    implicit none

#include "defin.cdk90"
      external ecritur, pgsmluk, ipgsmluk, fstprm, qaaqr, symetri, imprime, pgsmabt
      external cvtrfi,cvtifr
      integer  pgsmluk, ipgsmluk, fstprm, fstinl
...
#include "voir.cdk90"
    ...
    integer nniv, dateo, datev, i, nunv, key, ii, j
    integer niveaux(nniv), deet, ig1, ig2, ig3, ig4, iheur
    integer iprs, irec, iunit, jp1, jp2, jp3, ne, ni, nj, nk, nrecs
    ...
    do iprs = 1, nniv
        ...
        if (cnom == cnommt) then
            if (.not. mtdone) then
                irec = fstinl(iunit, ni, nj, nk, -1, '            ', -1, -1, -1, '  ', cnom, liste, nrecs, nRecords)
                if (nrecs == 0) then
                    if (message) then
            ...
            irec = fstinl(iunit, ni, nj, nk, datev, cetiket, niveaux(iprs), iheur, ip3ent, ctypvar, cnom, liste, nrecs, nRecords)
        ...
        do ii=1, nrecs
            ...
            if (cnomvar(1:2) == '>>'.or.cnomvar(1:2) == '^^'.or.cnomvar(1:2) == 'HY'.or.cnomvar == '^>'.or.cnomvar == '!!') cycle

            if (ctypvar(2:2) == '@'.and.masque == 1) then
                if (ctypvar(1:1) /= '@') then
                    call scalair_msk(irec, liste, done_fields,nRecords)
                endif
                cycle
            endif
            ...
            if (cdatyp == 2 .or. cdatyp == 4.or.cdatyp == 130.or.cdatyp == 132) then
            ...
            if (.not. skip) then
                if (cigtyp /= cgrtyp .or. ig1 /= lg1 .or. ig2 /= lg2 .or. ig3 /= lg3 .or. ig4 /= lg4 .or. li /= ni .or. lj /= nj) then
                    ! interpolation
