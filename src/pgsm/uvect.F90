...
subroutine uvectur (cnom1, cnom2, cnom3, iheur, npar, itabuv)
   ...
   external ecritur, cigaxg, fstinf, fstinl, ipgsmlic, pgsmlic, ipgsmlir, pgsmlir, fstprm, pgsmabt, imprime, vdauv, incdat, fstopc, messags
   external liraxez, cxgaig
   integer fstinf, fstinl, ipgsmlic, pgsmlic, ipgsmlir, pgsmlir, fstprm, fstopc
...
#include "tp12ig.cdk90"
    ...
    do iprs = 1, npar
        ...
        ! ier = fstinl(iunit, ni, nj, nk, datev, cetiket, itabuv(iprs), iheur, ip3ent, ctypvar, cnom1, listniv, infon, nRecords)
        query = inputFiles(1)%new_query(datev = datev, etiket = cetiket, ip1 = itabuv(iprs), ip2 = iheur, ip3 = ip3ent, typevar = ctypvar, nomvar = cnom1)
 ...
 610     format('uvectur: No record in file NOM=', a2)
        ...
        do ilop = 1, infon
            ...
            if (records(ilop)%grtyp == 'G' .and. mod(records(ilop)%ni, 2) /= 0)  call messags(ni)
                ...
                vv_query = inputFiles(1)%new_query(datev = datdv, etiket = cetike, ip1 = jp1, ip2 = jp2, ip3 = jp3, typevar = ctypvar, nomvar = cnom2)
                if (irec_vv < 0) then
                ...
                if (cigtyp == 'G'.and.mod(ni, 2) /= 0)  call messags(ni)
