!> Extraire l'exponentiel de chaque point d un champ dans accumulateur et multiplier le resultat par le fact
subroutine operat(fact, ecart, divi)
    use app
    use accum, only : nni, nnj, icnt
    use pgsm_mod, only: tmpif0, message
    use chck, only : ichck
    implicit none
    ...
    !APPEL    VIA DIRECTIVE
    !          PFOIS(FACT, ECART, DIVI) - EXPON(FACT)- MOYENE(FACT) - ALOGN(FACT) - RACINE(FACT)
