...
!
 ...
 10            format(a2, 2x, a4, a12, a1, 3x)
            ...
            call pgsmwr(2, 1, fld, ni, nj, nk, qcform, qposition, qitems, qcsepar, cnomvar, ctypvar, cetiket, &
                idat, idatv, dateform, ip1, ip2, ip3, tmplat, tmplon)
         else
            if (message) then
               call app_log(APP_ERROR, 'ecritur: Unknown output file type!')
            endif
         endif
      endif
...
#include "lnkflds.cdk90"
    ...
    if (mode .eq. 1) then
        ...
        if (mode .eq. 2) then
            ...
            ier = gdll(gdout, tmplat, tmplon)
            call pgsmwr(2, 0, fld, ni, nj, nk, qcform, qposition, qitems, qcsepar, cnomvar, ctypvar, cetiket, &
                idat, idatv, dateform, ip1, ip2, ip3, tmplat, tmplon)
        else
            if (message) then
            call app_log(APP_WARNING, 'ecritur: Unknown output file type!')
            endif
        endif
    endif
