...
***   Lire l'information de controle se trouvant dans IUN.
      ...
      inf  = fstinf( IUN, NIC,NJC,NKC, -1,'CONTROLE',
     +               -1,-1,-1,'+','T/' )
     ...
     +    iluk        .lt. 0       )                   Then
        Write(6,6001) NIC,NJC,NKC
        nil = exfin( 'LITZON','Erreur 1','NON' )
        Call qqexit( 1 )
      End If
...
*     ne sont ecrits que pour le modele sef  
      ...
      If(iluk  .ge. 0       )                            Then
        ...
        If (NIC*NJC*NKC .gt. Maxbin  )                   Then
          Write(6,6010) NIC,NJC,NKC
          nil = exfin( 'LITZON','Erreur 10','NON' )
          Call qqexit( 10 )
        End If
      End If
...
***   Lire les niveaux sigma du modele                       
     ...
     +               -1,-1,-1, '+','S^'  )
      ...
      if (NIC*NJC*NKC .gt. Maxniv .or.
     +    iluk        .lt. 0         )                   Then
        Write(6,6011) NIC,NJC,NKC
        nil = exfin( 'LITZON','Erreur 11','NON' )
        Call qqexit( 11 )
      End If
...
***   Check si conforme au nombre dans table d'information 
      ...
      if (NSIG .ne. NK)                                  Then 
        Write(6,6012) NSIG,NK
        nil = exfin( 'LITZON','Erreur 12','NON' )
        Call qqexit( 12 )
      End If
...
***   Lire Somx et Somx2 (nbin,somnk)
      ...
      If (mod(MODE,2).eq.1)                               Then
        ...
        inf  = fstinf( IUN, NIC,NJC,NKC, -1,'MZONXST ',
     +                 ip1,ip2,ip3, '+','1/' )
...
***   Lire tous les parametres du fichier standard de controle.
     ...
     +                 ip1,ip2,ip3, '+','2/' )
        ...
        If ( iluk .lt. 0  .and. heusav .eq. 0 )            Then
          Write(6,6009) NIC,NJC,NKC
          nil = exfin( 'LITZON','Erreur 9','NON' )
          Call qqexit( 9 )
        End If
...
***   Fermer le fichier IUN
      ...
      If (inbr.lt. 0)                                      Then
        Write(6,6015)
        nil = exfin( 'LITZON','Erreur 15','NON' )
        Call qqexit( 15 )
      End If
