...
.F90.o:
	Verbose=yes s.f90 -c $(OPTIMIZ) $<

.c.o:
	Verbose=yes s.cc -c $(OPTIMIZ) $<
...
bemol: $(OBJECTS)
	s.f90 -o bemol_$(VER)-$(BASE_ARCH) $(OBJECTS) $(OPTIMIZ) -l$(LIBRMN)

zcrop:
	(cd utils/zcrop; s.f90 -o zcrop_$(VERZC)-$(BASE_ARCH) zcrop.F90 $(OPTIMIZ) -l$(LIBRMN))
	(cd utils/zcrop; mv zcrop_$(VERZC)-$(BASE_ARCH) ../..)

bemol+: $(OBJECTS)
	s.f90 -o bemol+ $(OBJECTS) -lrmnbeta
